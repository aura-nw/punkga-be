...
export class IPFSService implements OnModuleInit {
  ...
  async uploadLocalFolderToIpfs(localFolderPath: string, ipfsFolderPath: string, chunkSize = 10) {
    const filenames = readdirSync(localFolderPath);
    if (_.isEmpty(filenames)) throw new Error(`Folder ${localFolderPath} is empty`);
    ...
    for (let i = 0; i < filenames.length; i += chunkSize) {
      const uploadPromisses = filenames
        ...
        .map((filename) => {
          ...
          return this.ipfsClient.files.write(ipfsFilePath, content, { create: true });
        });
  ...
  async uploadMetadataObjectsToIpfs(objects: IMetadata[], ipfsFolderPath: string, chunkSize = 10): Promise<string> {
    await this.createFolderIfNotExist(ipfsFolderPath);

    for (let i = 0; i < objects.length; i += chunkSize) {
      const uploadPromisses = objects
        .slice(i, i + chunkSize)
        .map((object) => {
  ...
  private async createFolderIfNotExist(ipfsPath: string) {
    ...
    if (!exists)
      return this.ipfsClient.files.mkdir(ipfsPath);
  }
